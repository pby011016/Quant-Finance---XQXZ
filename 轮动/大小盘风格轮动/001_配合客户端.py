...
df_big = pd.read_csv('510050.SH.csv', encoding='gbk', parse_dates=['交易日期'], skiprows=1)
df_small = pd.read_csv('159845.SZ.csv', encoding='gbk', parse_dates=['交易日期'], skiprows=1)

# 设置参数
trade_rate = 0.6 / 10000  # 场内基金万分之0.6，买卖手续费相同，无印花税
momentum_days = 20  # 计算多少天的动量
# 计算大小盘每天的涨跌幅amplitude
df_big['big_amp'] = df_big['收盘价'] / df_big['收盘价'].shift(1) - 1
df_small['small_amp'] = df_small['收盘价'] / df_small['收盘价'].shift(1) - 1
# 重命名行
df_big.rename(columns={'开盘价': 'big_open', '收盘价': 'big_close'}, inplace=True)
df_small.rename(columns={'开盘价': 'small_open', '收盘价': 'small_close'}, inplace=True)
# 合并数据
df = pd.merge(left=df_big[['交易日期', 'big_open', 'big_close', 'big_amp']], left_on=['交易日期'],
              right=df_small[['交易日期', 'small_open', 'small_close', 'small_amp']],
              right_on=['交易日期'], how='left')
# 设置起始日期
df['candle_end_time'] = pd.to_datetime(df['交易日期'])
df = df[df['candle_end_time'] >= '2016-01-01']
...
# df.loc[(df['big_mom'] > 0.1) | (df['small_mom'] > 0.1), 'style'] = 'empty'
# 相等时维持原来的仓位。
...
df = df[['candle_end_time', 'strategy_amp_adjust', 'strategy_net']]
df.to_csv('大小盘风格切换_净值.csv', encoding='gbk', index=False)