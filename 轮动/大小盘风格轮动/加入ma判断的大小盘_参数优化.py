...
df_big = pd.read_csv(r'd:\北京财兴智安实习\轮动\大小盘风格轮动\sh000016.csv', encoding='gbk', parse_dates=['candle_end_time'])
df_small = pd.read_csv(r'd:\北京财兴智安实习\轮动\大小盘风格轮动\sh000852.csv', encoding='gbk', parse_dates=['candle_end_time'])

# 设置参数
trade_rate = 0.6 / 10000  # 场内基金万分之0.6，买卖手续费相同，无印花税
momentum_days = 9  # 计算多少天的动量
ma_par = 20  # 计算多少天的长期动量
...
df = df[df['candle_end_time'] >= '2010-01-01']

for momentum_days in range(1, 21):
    for ma_par in range(5, 65, 5):
        # 计算N日的动量momentum
        df['big_mom'] = df['big_close'].pct_change(periods=momentum_days)
        df['small_mom'] = df['small_close'].pct_change(periods=momentum_days)
        df['big_ma'] = df['big_close'].rolling(window=ma_par, min_periods=1).mean()
        df['small_ma'] = df['small_close'].rolling(window=ma_par, min_periods=1).mean()
        ...
        df.loc[(df['big_ma'] < df['big_close']) & (df['small_ma'] < df['small_close']), 'style'] = 'empty'
        ...
        # 空仓的日子，涨跌幅用0填充
        df['strategy_amp_adjust'] = df['strategy_amp_adjust'].fillna(0.0)
        ...
        # print(res.loc['累积净值', 0])
        if res.loc['累积净值', 0] > 3.5:
            print('momentum_days:', momentum_days, 'ma_par:', ma_par)
            print(res)
