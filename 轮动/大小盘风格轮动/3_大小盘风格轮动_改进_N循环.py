...
for momentum_days in N:
    ...
    # df = df[df['candle_end_time'] >= '2023-12-01']
    # 设置起始和截止日期
    df['candle_end_time'] = pd.to_datetime(df['candle_end_time'])
    df = df[(df['candle_end_time'] >= '2023-01-01') & (df['candle_end_time'] <= '2024-12-31')]

    # 恢复df初始状态
    df = df[['candle_end_time', 'big_open', 'big_close', 'big_amp', 'small_open', 'small_close', 'small_amp']]
    # 计算N日的动量momentum
